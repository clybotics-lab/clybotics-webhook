...
bp = Blueprint("internal_instagram", __name__, url_prefix="/internal/v1/instagram")
_db = SupabaseRest()
...
def provision():
    ...
    page_id = _read_meta_str(meta, "instagramPageId", "instagram_page_id", "pageId", "page_id")
    ...
    _db.upsert_instagram_provision(ws, bot_id, page_id, verify)
    ...
    callback = f"{public_base}/v1/instagram/{bot_id}"
    return jsonify(
        {
            "ok": True,
...
def disconnect():
    ...
    _db.disconnect_instagram_channel(ws, bot_id)
