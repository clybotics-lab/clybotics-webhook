...
_RUNTIME_DIFY_BASE_CACHE: Optional[tuple[float, str]] = None


def _resolve_dify_api_base(db: SupabaseRest, meta_base: str, api_key: str) -> str:
    """
    Effective Dify API root for chat-messages: bot metadata base, else DIFY_DEFAULT_BASE_URL, else
    public.runtime_settings.dify_server_base_url (cached).
    """
    if meta_base.strip():
        return meta_base.strip()
    if not api_key.strip():
        return ""
    if DIFY_DEFAULT_BASE_URL:
        return DIFY_DEFAULT_BASE_URL
    global _RUNTIME_DIFY_BASE_CACHE
    now = time.monotonic()
    if _RUNTIME_DIFY_BASE_CACHE is not None and (now - _RUNTIME_DIFY_BASE_CACHE[0]) < DIFY_RUNTIME_BASE_CACHE_SECONDS:
        return _RUNTIME_DIFY_BASE_CACHE[1]
    raw = db.get_runtime_setting_text("dify_server_base_url")
    url = (raw or "").strip()
    _RUNTIME_DIFY_BASE_CACHE = (now, url)
    return url


def dify_fallback_health_info(db: SupabaseRest) -> dict[str, Any]:
    """Non-secret snapshot for GET /health?dify=1."""
    db_val = db.get_runtime_setting_text("dify_server_base_url")
    return {
        "env_dify_default_base_url_set": bool(DIFY_DEFAULT_BASE_URL),
        "db_dify_server_base_url_set": bool((db_val or "").strip()),
    }
